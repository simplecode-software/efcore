...
namespace Microsoft.EntityFrameworkCore.Query
{
        ...
        where TFixture : NorthwindQueryFixtureBase<NoopModelCustomizer>, new()
    {
        ...
        public virtual Task Join_select_many(bool async)
        {
            return AssertQuery(
                async,
                ss => from c in ss.Set<Customer>().Where(c => c.CustomerID.StartsWith("C"))
                      join o in ss.Set<Order>() on c.CustomerID equals o.CustomerID
        ...
        public virtual Task Join_composite_key(bool async)
        {
            return AssertQuery(
                async,
                ss =>
                    from c in ss.Set<Customer>().Where(c => c.CustomerID.StartsWith("B"))
        ...
        public virtual Task Join_same_collection_force_alias_uniquefication(bool async)
        {
            return AssertQuery(
                async,
                ss =>
                    ss.Set<Order>().Where(o => o.CustomerID.StartsWith("P")).Join(
        ...
        public virtual Task GroupJoin_simple(bool async)
        {
            return AssertQuery(
                async,
                ss =>
                    from c in ss.Set<Customer>().Where(c => c.CustomerID.StartsWith("S"))
        ...
        public virtual Task GroupJoin_simple_ordering(bool async)
        {
            return AssertQuery(
                async,
                ss =>
                    from c in ss.Set<Customer>().Where(c => c.CustomerID.StartsWith("A")).OrderBy(c => c.City)
                    join o in ss.Set<Order>() on c.CustomerID equals o.CustomerID into orders
                    from o in orders
                    select o,
                entryCount: 30);
        ...
        public virtual Task GroupJoin_projection(bool async)
        {
            return AssertQuery(
                async,
                ss =>
                    from c in ss.Set<Customer>().Where(c => c.CustomerID.StartsWith("M"))
                    join o in ss.Set<Order>() on c.CustomerID equals o.CustomerID into orders
                    from o in orders
                    select new { c, o },
                e => (e.c.CustomerID, e.o.OrderID),
        ...
        public virtual Task GroupJoin_DefaultIfEmpty(bool async)
        {
            return AssertQuery(
                async,
                ss =>
                    from c in ss.Set<Customer>().Where(c => c.CustomerID.StartsWith("L"))
                    join o in ss.Set<Order>() on c.CustomerID equals o.CustomerID into orders
        ...
        public virtual Task GroupJoin_DefaultIfEmpty2(bool async)
        {
            return AssertQuery(
                ...
                ss =>
                    from e in ss.Set<Employee>()
                    join o in ss.Set<Order>().Where(o => o.CustomerID.StartsWith("O")) on e.EmployeeID equals o.EmployeeID into orders
                    from o in orders.DefaultIfEmpty()
                    select new { e, o },
...
#pragma warning restore RCS1146 // Use conditional access.
        ...
        public virtual Task GroupJoin_SelectMany_subquery_with_filter_and_DefaultIfEmpty(bool async)
        {
            return AssertQuery(
                async,
                ss =>
                    from c in ss.Set<Customer>().Where(c => c.CustomerID.StartsWith("C"))
                    join o in ss.Set<Order>() on c.CustomerID equals o.CustomerID into lo
