...
WHERE [o].[CustomerID] IS NOT NULL AND ([o].[CustomerID] LIKE N'P%')");
        }
...
LEFT JOIN (
    SELECT [o].[OrderID], [o].[CustomerID], [o].[EmployeeID], [o].[OrderDate]
    FROM [Orders] AS [o]
    WHERE [o].[CustomerID] IS NOT NULL AND ([o].[CustomerID] LIKE N'O%')
